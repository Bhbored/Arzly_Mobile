...
class MobileAccessoriesStep1Body extends ConsumerStatefulWidget {
  const MobileAccessoriesStep1Body({
    super.key,
  ...
  @override
  ConsumerState<MobileAccessoriesStep1Body> createState() =>
      _MobileAccessoriesStep1BodyState();
}

class _MobileAccessoriesStep1BodyState
    extends ConsumerState<MobileAccessoriesStep1Body> {
  Future<void> _openBrandPicker() async {
    final picked = await Navigator.of(context).push<AccessoryBrand?>(
      createAccessoryBrandPickerRoute(),
    );
    if (picked != null && mounted) {
      ref.read(tempPhonesGadgetsDraftHolderProvider.notifier).update(
            (d) => d.copyWith(accessoryBrand: picked),
          );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final fieldsResetKey =
        Object.hash(details.accessoryBrand, details.phoneCondition);
        ...
        child: Column(
          ...
          children: [
                          ...
                          Expanded(
                            child: Text(
                              ...
                              style: Theme.of(context)
                                  ...
                                  .bodyLarge
                                  ?.copyWith(
                                    color: details.accessoryBrand == null
            ...
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Item type', style: titleStyle),
                SizedBox(height: context.spaceSmall),
                CarsForSaleStyleDropdownField<MobileAccessoryType>(
                  fieldsResetKey: fieldsResetKey,
                  fieldSuffix: 'accessoryItemType',
                  value: details.accessoryItemType,
                  onChanged: (v) => ref
                      .read(tempPhonesGadgetsDraftHolderProvider.notifier)
                      .update((d) => d.copyWith(accessoryItemType: v)),
                  hintText: 'Choose item type',
                  validationMessage: 'Choose item type',
                  showRequiredErrors: widget.requireFieldErrors,
                  pageBg: pageBg,
                  scheme: scheme,
                  menuRadius: menuRadius,
                  items: [
                    for (final t in MobileAccessoryType.values)
