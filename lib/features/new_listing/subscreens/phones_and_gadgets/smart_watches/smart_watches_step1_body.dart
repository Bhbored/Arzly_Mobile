...
class SmartWatchesStep1Body extends ConsumerStatefulWidget {
  const SmartWatchesStep1Body({
    super.key,
  ...
  @override
  ConsumerState<SmartWatchesStep1Body> createState() =>
      _SmartWatchesStep1BodyState();
}

class _SmartWatchesStep1BodyState extends ConsumerState<SmartWatchesStep1Body> {
  Future<void> _openBrandPicker() async {
    final picked = await Navigator.of(context).push<WatchBrand?>(
      createWatchBrandPickerRoute(),
    );
    if (picked != null && mounted) {
      ref.read(tempPhonesGadgetsDraftHolderProvider.notifier).update(
            (d) => d.copyWith(watchBrand: picked),
          );
    }
  }

  List<DropdownMenuItem<BandColor>> _bandColorMenuItems(ColorScheme scheme) {
    return [
      for (final c in BandColor.values)
        DropdownMenuItem(
          value: c,
          child: Row(
            children: [
  ...
  Widget build(BuildContext context) {
    ...
    final fieldsResetKey = Object.hash(
      details.watchBrand,
      details.phoneCondition,
      details.bandMaterial,
    );
          ...
          children: [
            Column(
              ...
              children: [
                    ...
                    child: InputDecorator(
                      ...
                      child: Row(
                        children: [
                          Expanded(
                            child: Text(
                              details.watchBrand?.label ?? 'Choose brand',
                              maxLines: 1,
                ...
                if (widget.requireFieldErrors && details.watchBrand == null)
                  Padding(
            ...
            Column(
              ...
              children: [
                ...
                if (widget.requireFieldErrors &&
                    details.phoneCondition == null)
                  Padding(
                    padding: EdgeInsets.only(top: context.spaceSmall * 0.5),
                    child: Text(
                      'Choose condition',
            ...
            Column(
              ...
              children: [
                ...
                SizedBox(height: context.spaceSmall),
                CarsForSaleStyleDropdownField<WatchStorage>(
                  fieldsResetKey: fieldsResetKey,
                  fieldSuffix: 'watchStorage',
                  value: details.watchStorage,
                  onChanged: (v) => ref
                      .read(tempPhonesGadgetsDraftHolderProvider.notifier)
                      .update((d) => d.copyWith(watchStorage: v)),
                  ...
                  items: [
                    for (final s in WatchStorage.values)
            ...
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Band material', style: titleStyle),
                SizedBox(height: context.spaceSmall),
                CarsForSaleStyleDropdownField<BandMaterial>(
                  fieldsResetKey: fieldsResetKey,
                  fieldSuffix: 'bandMaterial',
                  value: details.bandMaterial,
                  onChanged: (v) => ref
                      .read(tempPhonesGadgetsDraftHolderProvider.notifier)
                      .update((d) => d.copyWith(bandMaterial: v)),
                  hintText: 'Choose band material',
                  validationMessage: 'Choose band material',
                  showRequiredErrors: widget.requireFieldErrors,
                  pageBg: pageBg,
                  scheme: scheme,
                  menuRadius: menuRadius,
                  items: [
                    for (final m in BandMaterial.values)
            ...
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Band color', style: titleStyle),
                SizedBox(height: context.spaceSmall),
                CarsForSaleStyleDropdownField<BandColor>(
                  fieldsResetKey: fieldsResetKey,
                  fieldSuffix: 'bandColor',
                  value: details.bandColor,
                  onChanged: (v) => ref
                      .read(tempPhonesGadgetsDraftHolderProvider.notifier)
                      .update((d) => d.copyWith(bandColor: v)),
                  hintText: 'Choose band color',
                  validationMessage: 'Choose band color',
                  showRequiredErrors: widget.requireFieldErrors,
                  pageBg: pageBg,
                  scheme: scheme,
                  menuRadius: menuRadius,
                  items: _bandColorMenuItems(scheme),
                ),
              ],
            ),
          ],
