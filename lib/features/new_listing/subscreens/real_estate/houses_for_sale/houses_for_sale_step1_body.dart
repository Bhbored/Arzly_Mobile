...
class HousesForSaleStep1Body extends ConsumerStatefulWidget {
  const HousesForSaleStep1Body({
    super.key,
  ...
  @override
  ConsumerState<HousesForSaleStep1Body> createState() =>
      _HousesForSaleStep1BodyState();
}

class _HousesForSaleStep1BodyState extends ConsumerState<HousesForSaleStep1Body> {
  ...
  void initState() {
    ...
    _sizeController.addListener(() {
      ref.read(tempRealEstateDraftHolderProvider.notifier).update(
            (d) => d.copyWith(
              size: parseHousesSizeM2Field(_sizeController.text),
            ),
          );
    });
  ...
  Future<void> _openFeatures() async {
    ...
    if (picked != null && mounted) {
      ref.read(tempRealEstateDraftHolderProvider.notifier).update(
            (d) => d.copyWith(features: picked.isEmpty ? null : picked),
          );
    }
  }
